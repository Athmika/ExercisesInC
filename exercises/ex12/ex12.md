...
There are syncronization errors because all the threads can run concurrently. However, the final of the counter will be correct
because the program terminates after executing a certain number of children and each child only increments the counter be one. 
...
The syncronization makes it significantly slower because the locking and unlocking of the mutex datastructure takes significant overhear.
 In my tests it made it 10 times slower.
